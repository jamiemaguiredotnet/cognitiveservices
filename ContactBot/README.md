...
# To try this sample
- In a terminal, navigate to `<your_project_folder>/ContactAssistant`
    ```bash
    # change into project folder
	cd <your_project_folder>/ContactAssistant
    ```
- Setup LUIS
    ...
    - As the command `msbot clone services` uses secret-manager to safely store the key generated for encrypting the bot file, it's necessary adding the next code in your ContactAssistant.csproj file:
    ...
    **Note**: You can add any arbitrary `GUID` value for the `UserSecretsId` property, but you can not repeat `GUID` values among the projects, as they must be unique.
    ...
    msbot clone services --name "ContactAssistant" --luisAuthoringKey <LUIS-authoring-key> --code-dir "." --location westus --sdkLanguage "CSharp" --folder deploymentScripts/msbotClone --verbose
    ```

    **Note**: Once the Luis service application is created, change the value of `LuisConfiguration` variable in ContactAssistantBot.cs with the Luis service name, which can be found in ContactAssistant.bot file (it should be `ContactAssistant_core-bot-LUIS`).
...
- Run the bot from a terminal or from Visual Studio, choose option A or B.
	...
	- Select `ContactAssistant.csproj` file
...
msbot clone services --name "ContactAssistant" --code-dir "." --location westus --sdkLanguage "Csharp" --folder deploymentScripts/msbotClone --verbose
```
